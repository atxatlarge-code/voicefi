...
def main():
    ...
    states_to_capture = [
        ...
        ("working", lambda: hud.set_working(agent_name="Antigravity", tool_action="Executing: pytest tests/ -v (Passed 210/210)")),
        ("speaking", lambda: hud.set_speaking(text="All 12 unit tests passed. Ready to deploy. Staging on Railway now...", agent_name="Antigravity", persona_name="Viv")),
        ("listening", lambda: hud.set_listening(prompt_preview="", user_name="Jake")),
        ("listening_stream", lambda: hud.set_listening(prompt_preview="Fix the expired token bug in auth_service.py", user_name="Jake", live_stream=True)),
        ("new_conversation", lambda: hud.set_new_conversation(prompt_preview="Start a new session with Google Search and MCP tools", user_name="Jake", live_stream=True)),
        ("editing", lambda: hud.set_editing(initial_text="Refactor authentication controller to use JWT tokens", on_submit=lambda x: None, target_name="Antigravity")),
    ]
