...
def main():
    ...
    parser.add_argument(
        ...
        default="1",
        choices=["1", "2"],
        help="Line to punch in (1=Antigravity 6mo, 2=PMP/TBI question)",
    )
    parser.add_argument(
        "-i",
        "--input-wav",
        type=str,
        default=None,
        help="Optional pre-recorded WAV file to splice in directly",
    )
    ...
    music_path = (
        ROOT_DIR / "assets" / "audio" / "backing_tracks" / "track_05_bittersweet_symphony.mp3"
    )
    ...
    if args.input_wav:
        ...
        print(
            f"   We will play ~5s of the backing track lead-in ({target['pre_roll']:.1f}s) to catch the groove."
        )
    ...
    subprocess.run(
        [
            ...
            str(punch_wav),
            "-af",
            "highpass=f=85,silenceremove=start_periods=1:start_threshold=-40dB:start_silence=0.05",
            "-ar",
            str(SAMPLE_RATE),
            "-ac",
            "1",
