...
def record_from_microphone(output_wav: Path, max_seconds: float = 30.0) -> bool:
    ...
    print("Example lines:")
    print(
        " • “Look, I just wanted to pace around my room and talk to my code without typing all day!”"
    )
    print(
        " • “For years, coding was silent. Staring at terminal logs. So we gave our agents a real voice.”"
    )
    print(
        " • “Once you start pair-programming with your agents by voice, you can never go back.”\n"
    )
...
def restore_user_vocal(
    input_audio_path: Path, output_wav: Path, sample_rate: int = SAMPLE_RATE
) -> np.ndarray:
    ...
    if len(speech_indices) > 0:
        ...
        last_idx = min(
            len(raw), speech_indices[-1] * win + win + int(0.30 * sample_rate)
        )  # 300ms lead-out
...
def main():
    ...
    parser.add_argument(
        "-i", "--input", type=str, help="Path to existing audio file (skips mic recording)"
    )
    parser.add_argument(
        "-o", "--output", type=str, default="assets/jake_voice.wav", help="Output restored WAV path"
    )
