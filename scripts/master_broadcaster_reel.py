...
def replace_dialogue_window(dialogue, replacement, room_tone, window_start_t, window_end_t, sr, fade_len=960):
    """
    ...
    dialogue[start_idx : start_idx + fade_len] = (
        dialogue[start_idx : start_idx + fade_len] * (1.0 - fade_in) +
        splice[:fade_len] * fade_in
    )
...
def assemble():
    ...
    run(f'ffmpeg -y -ss 00:00:00 -t {outro_transition_t:.2f} -i "{SPICEWOOD_BEAT}" -ar {sr} -ac 2 "{beat_wav}"')
    ...
    beat_gain_envelope[doc_start_idx - ramp_len : doc_start_idx] = np.linspace(10**(-21.0 / 20.0), 0.0, ramp_len)
    beat_gain_envelope[doc_start_idx : doc_end_idx] = 0.0
    beat_gain_envelope[doc_end_idx : doc_end_idx + ramp_len] = np.linspace(0.0, 10**(-21.0 / 20.0), ramp_len)
    ...
    run(f'ffmpeg -y -ss 00:00:55.50 -to 00:01:01.35 -i "{SPICEWOOD_MASTER}" -ar {sr} -ac 2 "{hook_wav}"')
