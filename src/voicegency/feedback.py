"""
Feedback and diagnostics subsystem for Voicegency.
...
from voicegency.config import load_config, get_default_config_path


def get_feedback_dir() -> Path:
    """Return the feedback directory (~/.voicegency/feedback/)."""
    fb_dir = Path.home() / ".voicegency" / "feedback"
...
def collect_system_diagnostics() -> Dict[str, Any]:
    ...
    diagnostics: Dict[str, Any] = {
        "voicegency_version": __version__,
...
def submit_feedback(
    ...
    log_path = Path.home() / ".voicegency" / "feedback.jsonl"
