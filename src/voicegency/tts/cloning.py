"""
Voice Training, Acoustic Analysis, and Instant Voice Cloning Manager for Voicegency.
...
from voicegency.config import AgentVoiceProfile, VoicegencyConfig, save_config
from voicegency.tts.elevenlabs import ElevenLabsTTS
...
def get_clones_dir() -> Path:
    """Return the root directory for cloned voices (~/.voicegency/cloned_voices)."""
    d = Path.home() / ".voicegency" / "cloned_voices"
...
class VoiceCloneManager:
    ...
    def train_voice(
        ...
        - Ingests samples to ~/.voicegency/cloned_voices/<name>/samples/
        ...
        labels_dict.setdefault("cloned_by", "voicegency")
        ...
        if api_key:
            try:
                resp = ElevenLabsTTS.add_voice(
                    ...
                    description=description or f"Custom cloned voice of {clean_name} (Voicegency)",
                    labels=labels_dict,
                )
    ...
    def assign_to_agent(
        ...
        """
        Assign the cloned voice to an agent or subagent in VoicegencyConfig and save.
