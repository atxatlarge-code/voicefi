...
def send_message_to_antigravity(conv_id: Optional[str] = None, text: str = "") -> bool:
    """
    ...
    if not conv_id:
        try:
            from voicegency.integrations.conversations import ConversationTracker
    ...
    if agentapi_bin.is_file() and os.access(agentapi_bin, os.X_OK) and conv_id:
        try:
            res = subprocess.run(
                [str(agentapi_bin), "send-message", str(conv_id), clean_text],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
                timeout=5,
            ...
            if res.returncode == 0:
                print(f"[Injector] 🚀 Delivered prompt directly via agentapi IPC to {conv_id[:8]} (zero window flicker)")
                return True
            else:
