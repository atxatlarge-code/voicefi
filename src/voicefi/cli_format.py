...
COMMAND_CATEGORIES = [
    ...
    (
        "⚙️ Server & Management",
        [
            ("status", "Show VoiceFi server status, active devices, and port listeners"),
            ("server", "Inspect and manage background server, LaunchAgents, and ports"),
            ("stop / restart", "Stop or restart background VoiceFi server & free port 5141"),
            ("clean", "Purge __pycache__, stale locks, caches, and orphaned servers"),
            ("dev", "Live foreground dev mode with real-time logs and auto-takeover"),
            ("update", "Check for and install latest VoiceFi updates"),
            ("autostart", "Enable background LaunchAgent server (vifi tray)"),
            ("stop-autostart", "Remove background LaunchAgent server"),
