"""
Voice Control Panel and Voice Command Engine for VoiceFi.
...
def parse_voice_command(command_text: str, config: VoiceFiConfig) -> Dict[str, Any]:
    ...
    if (audition_match or "how does" in text) and target_persona_name:
        ...
        sample_text = (
            ...
            else f"Hello! This is {target_persona_name} auditioning for VoiceFi."
        )
        provider = target_persona_obj.provider if target_persona_obj else "edge_tts"
...
def get_current_system_state(config: VoiceFiConfig) -> Dict[str, Any]:
...
<head>
  ...
  <title>VoiceFi • Voice Control Panel</title>
  <style>
    :root {
      --bg-color: #f8fafc;
      --bg-surface: #ffffff;
...
<div class="container">
  ...
  <header>
    <div class="logo-group">
      ...
      <div class="logo-text">
        <h1>VoiceFi Control Panel <span class="badge-patent">Patent Pending</span></h1>
        <p>A Voice For Every Agent • Multi-Agent Auditions & Hands-Free Voice Control</p>
      </div>
    </div>
  ...
  <div class="section-title">
    <span>A Voice For Every Agent (Curated Personas)</span>
  ...
  <div class="tuning-box">
    ...
    <div class="preset-chips">
      <span style="font-size: 12px; font-weight: 600; color: var(--text-muted); align-self: center; margin-right: 4px;">Preset Phrases:</span>
      <span class="preset-chip" onclick="setPresetPhrase('greeting')">👋 Quick Greeting</span>
      <span class="preset-chip" onclick="setPresetPhrase('code_review')">💻 Code & DB Refactor</span>
      <span class="preset-chip" onclick="setPresetPhrase('qa_alert')">⚡ Fast QA Alert</span>
      <span class="preset-chip" onclick="setPresetPhrase('punctuation')">🔢 Numbers & Symbols</span>
      <span class="preset-chip" onclick="setPresetPhrase('architecture')">🧠 Architecture Summary</span>
    </div>
      ...
      <div>
        <div class="slider-group">
          <div class="slider-header">
            ...
            <span id="rateLabel">200 WPM (100%)</span>
          </div>
  ...
  <!-- Audio & Voice Diagnostic Troubleshooter -->
  <div class="troubleshoot-box" id="troubleshootSection">
    <div class="section-title">
      <span>🔊 Audio & Voice Diagnostic Troubleshooter</span>
      <span style="font-size: 13px; color: var(--green); font-weight: 600;">Hardware & Latency Inspector</span>
    </div>
    <p style="font-size: 13px; color: var(--text-muted); margin-bottom: 16px; line-height: 1.4;">
      Test speaker output, record microphone loopback to hear your own voice quality, benchmark neural voice TTFB latency, and resolve audio issues.
    </p>

    <div class="diag-grid">
      <!-- Card 1: Speaker Output Test -->
      <div class="diag-card">
        <div>
          <h4>🔔 Speaker Output & Chimes</h4>
          <p>Play acoustic notification chimes through your default speakers or headphones to confirm output.</p>
          <div style="display: flex; gap: 8px; flex-wrap: wrap;">
            <button class="btn-audition" style="padding: 6px 12px; font-size: 12px;" onclick="playTestChime('start')">
              ▶ Start Chime
            </button>
            <button class="btn-audition" style="padding: 6px 12px; font-size: 12px;" onclick="playTestChime('sent')">
              ▶ Sent Chime
            </button>
            <button class="btn-audition" style="padding: 6px 12px; font-size: 12px;" onclick="playTestChime('alert')">
              ▶ Alert Tone
            </button>
          </div>
        </div>
        <div id="speakerTestStatus" style="font-size: 12px; color: var(--text-muted); margin-top: 10px;">
          Status: Ready to test
        </div>
      </div>

      <!-- Card 2: Microphone Loopback Test -->
      <div class="diag-card">
        <div>
          <h4>🎙️ Mic Loopback (Hear Yourself)</h4>
          <p>Record 3 seconds of your voice. Hear playback immediately to check volume, clarity, and noise floor.</p>
          <div style="display: flex; gap: 8px; align-items: center; margin-bottom: 8px;">
            <button id="btnMicLoopback" class="btn-record" style="font-size: 12px; padding: 7px 14px;" onclick="runMicLoopbackTest()">
              🔴 Record 3s & Hear Playback
            </button>
          </div>
          <div class="diag-meter">
            <div class="diag-meter-fill" id="micEnergyMeter"></div>
          </div>
          <div id="micLoopbackAudioContainer" style="display: none; margin-top: 6px;">
            <audio id="micLoopbackAudio" controls style="width: 100%; height: 32px;"></audio>
          </div>
        </div>
        <div id="micLoopbackStatus" style="font-size: 12px; color: var(--text-muted); margin-top: 6px;">
          Status: Ready to record
        </div>
      </div>

      <!-- Card 3: TTS Latency Benchmark -->
      <div class="diag-card">
        <div>
          <h4>⚡ Neural TTS Latency Benchmark</h4>
          <p>Benchmark Time-to-First-Byte (TTFB) latency and cloud connectivity across all voice engines.</p>
          <button class="btn-audition" style="padding: 6px 14px; font-size: 12px;" onclick="runTTSBenchmark()">
            ⚡ Benchmark All Voices
          </button>
          <div id="benchmarkContainer" style="max-height: 140px; overflow-y: auto; margin-top: 10px;">
            <table class="benchmark-table" id="benchmarkTable">
              <thead>
                <tr><th>Voice</th><th>Provider</th><th>Latency</th></tr>
              </thead>
              <tbody id="benchmarkBody">
                <tr><td colspan="3" style="color: var(--text-muted); text-align: center;">Click Benchmark to test latency</td></tr>
              </tbody>
            </table>
          </div>
        </div>
      ...
      <!-- Card 4: Hardware & Settings Diagnostics -->
      <div class="diag-card">
        <div>
          <h4>🔍 System Hardware & Health</h4>
          <p>Detected audio devices, sample rates, and active VAD configuration.</p>
        ...
        <div style="margin-top: 12px; display: flex; gap: 6px; flex-wrap: wrap;">
          ...
          <button class="btn-fix" onclick="applyTroubleshootFix('set_offline_fallback')">
            ⚡ Offline Mac Voice
          </button>
          <button class="btn-fix" onclick="applyTroubleshootFix('reset_audio_defaults')">
            🔄 Reset Defaults
          </button>
        </div>
      </div>
...
<script>
  ...
  function setPresetPhrase(key) {
    if (PRESET_PHRASES[key]) {
      document.getElementById('customSampleText').value = PRESET_PHRASES[key];
      showToast(`📝 Selected preset: ${key.replace('_', ' ')}`);
    }
  }

  async function playTestChime(chimeName) {
    const statusEl = document.getElementById('speakerTestStatus');
    statusEl.innerHTML = `⏳ Playing '${chimeName}' chime...`;
    try {
      const res = await fetch('/api/troubleshoot/test_chime', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chime: chimeName })
      });
      const data = await res.json();
      if (data.success) {
        statusEl.innerHTML = `🟢 '${chimeName}' played successfully (${data.latency_ms} ms)`;
        showToast(`🔔 Played '${chimeName}' chime over speakers`);
      } else {
        statusEl.innerHTML = `🔴 Error: ${data.error || 'Failed'}`;
      }
    } catch (e) {
      statusEl.innerHTML = `🔴 Network error: ${e}`;
    }
  }

  async function runMicLoopbackTest() {
    const btn = document.getElementById('btnMicLoopback');
    const statusEl = document.getElementById('micLoopbackStatus');
    const meter = document.getElementById('micEnergyMeter');
    const audioContainer = document.getElementById('micLoopbackAudioContainer');
    const audioElement = document.getElementById('micLoopbackAudio');

    btn.disabled = true;
    btn.classList.add('recording');
    btn.textContent = '🗣️ Recording 3s... (Speak now)';
    statusEl.innerHTML = '🗣️ <b>Listening & recording...</b> Speak into your microphone';
    meter.style.width = '100%';
    meter.style.background = '#ef4444';

    try {
      const res = await fetch('/api/troubleshoot/mic_loopback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ duration: 3.0, play_back: true })
      });
      const data = await res.json();
      if (data.success) {
        meter.style.background = '#059669';
        const energyPct = Math.min(Math.round(data.rms_energy * 2500), 100);
        meter.style.width = `${Math.max(energyPct, 15)}%`;
        const speechDesc = data.speech_detected ? '✅ Clear Speech Detected' : '⚠️ Quiet Audio Level';
        statusEl.innerHTML = `🟢 <b>${speechDesc}</b> • RMS: ${data.rms_energy} • SNR: ${data.snr_db} dB`;
        if (data.base64_wav) {
          audioElement.src = data.base64_wav;
          audioContainer.style.display = 'block';
          audioElement.play().catch(() => {});
        }
        showToast('🔊 Microphone loopback playing through speakers!');
      } else {
        statusEl.innerHTML = `🔴 Mic Error: ${data.error || 'Failed'}`;
        meter.style.width = '0%';
      }
    } catch (e) {
      statusEl.innerHTML = `🔴 Error: ${e}`;
      meter.style.width = '0%';
    } finally {
      btn.disabled = false;
      btn.classList.remove('recording');
      btn.textContent = '🔴 Record 3s & Hear Playback';
    }
  }

  async function runTTSBenchmark() {
    const tbody = document.getElementById('benchmarkBody');
    tbody.innerHTML = '<tr><td colspan="3" style="text-align: center; color: var(--text-muted);">⏳ Measuring Time-to-First-Byte (TTFB) latency...</td></tr>';
    showToast('⚡ Benchmarking neural voice engines and measuring latency...');
    try {
      const res = await fetch('/api/troubleshoot/benchmark', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
      });
      ...
      if (data.benchmarks && data.benchmarks.length > 0) {
        tbody.innerHTML = data.benchmarks.map(b => {
          const isOk = b.status === 'online';
          const badge = isOk ? `<span style="color: #059669; font-weight: 600;">🟢 ${b.latency_ms} ms</span>` : `<span style="color: #ef4444; font-weight: 600;">🔴 Error</span>`;
          return `<tr><td><b>${b.name}</b></td><td>${b.provider}</td><td>${badge}</td></tr>`;
        }).join('');
        showToast('✅ Benchmark complete!');
      } else {
        tbody.innerHTML = '<tr><td colspan="3" style="color: #ef4444;">No benchmark results returned</td></tr>';
      }
    } catch (e) {
      tbody.innerHTML = `<tr><td colspan="3" style="color: #ef4444;">Benchmark failed: ${e}</td></tr>`;
    }
  }

  async function fetchDiagnostics() {
    try {
      const res = await fetch('/api/troubleshoot/diagnostics');
      const data = await res.json();
      if (data && data.hardware) {
        const hw = data.hardware;
        document.getElementById('hwDefaultInput').textContent = hw.default_input || 'None detected';
        document.getElementById('hwDefaultOutput').textContent = hw.default_output || 'None detected';
        document.getElementById('hwVadMode').textContent = hw.vad_mode || 'hybrid';
        document.getElementById('hwVadThresh').textContent = hw.vad_threshold || '0.004';
        document.getElementById('hwRatePct').textContent = `${hw.tts_rate_pct || '100%'} (${hw.tts_rate || 200} WPM)`;
        if (hw.tts_rate) {
          const slider = document.getElementById('rateSlider');
          if (slider) slider.value = hw.tts_rate;
          const label = document.getElementById('rateLabel');
          if (label) label.textContent = `${hw.tts_rate} WPM (${hw.tts_rate_pct || '100%'})`;
        }
      }
    } catch (e) {
  ...
  async function applyTroubleshootFix(fixType) {
    showToast(`🔧 Applying auto-fix: ${fixType.replace('_', ' ')}...`);
    try {
      const res = await fetch('/api/troubleshoot/fix', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        ...
        showToast(`⚠️ Fix notice: ${data.message}`);
      }
    } catch (e) {
      showToast(`❌ Error: ${e}`);
    }
...
class VoicePanelRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/troubleshoot/test_voice":
            ...
            res = AudioTroubleshooter(self.server.config).test_voice(
        ...
        if path == "/api/audition":
            ...
            persona = find_persona(voice)
            if not text:
                text = persona.sample_text if persona else f"Testing voice {voice} with VoiceFi."
        ...
        if path == "/api/voice_command":
            ...
            if result.get("action") == "audition":
                ...
                stext = result.get("sample_text", "Hello from VoiceFi.")
...
def open_control_panel(
    ...
    if open_browser:
        try:
            ...
            print(f"[VoiceFi] Could not automatically open browser: {e}")
    return url
