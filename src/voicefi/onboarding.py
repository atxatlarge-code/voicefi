...
def check_and_prompt_permissions() -> bool:
    ...
    try:
        ...
        if not trusted:
            ...
            print(
                "   VoiceFi uses macOS Accessibility for the universal <Esc> stop key and <Ctrl>+T dictation."
            )
...
def run_onboarding():
    ...
    if target_agent == "antigravity":
        ...
        print("   -> Event: VoiceFi Initial Setup Complete")
        ...
        try:
            ...
            if not delivered:
                cid = create_new_antigravity_conversation(
                    prompt=payload, title=f"VoiceFi Welcome: {extracted_name}"
                )
                delivered = bool(cid)
            ...
            if delivered:
                ...
                print(
                    "✨ Antigravity will now respond directly in your IDE (and VoiceFi will speak its reply when it finishes!)."
                )
            else:
        ...
        payload = (
            f"VoiceFi setup complete for {extracted_name}. Reason: {text2}. Ready to pair program!"
        )
        try:
            ...
            if delivered:
                ...
                print(
                    "✨ Claude Code will respond directly in your terminal (and VoiceFi will speak its reply!)."
                )
            else:
                print(
                    "💡 Claude Code terminal window not detected. Start Claude with 'claude' in any terminal!"
                )
        except Exception as e:
