"""
Unified Daemon, Process Lifecycle, and Cache Management for VoiceFi.
    ...
    try:
        ...
        if res.returncode == 0:
            for line in res.stdout.splitlines():
                ...
                # Match voicefi / vifi CLI, daemons, HUDs, or test runners
                cmd_lower = cmd.lower()
                is_voicefi = False
...
def get_full_daemon_status() -> Dict[str, Any]:
    """Compile comprehensive status of all VoiceFi daemons, processes, ports, and locks."""
...
def stop_all_voicefi_daemons(
    disable_launchagent: bool = True,
    remove_plist: bool = False,
    timeout_seconds: float = 3.0,
) -> Dict[str, Any]:
    """
    Safely and comprehensively stop all running VoiceFi daemons, background agents,
...
def clean_caches(
    ...
    """
    Clean stale Python bytecode, temporary daemon states, and update check caches.
    """
    ...
    if purge_daemons:
        daemons_stopped = stop_all_voicefi_daemons()
