...
class CompanionServer:
    ...
    async def handle_send(self, request: web.Request) -> web.Response:
        try:
            ...
            if intent_mode in ("speed", "comic") or target_engine in ("speed", "comic", "gemini_live"):
    ...
    async def handle_live(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            self.broadcast_event({
                ...
                "text": "Puck is roasting..." if intent_mode == "comic" else "Gemini Live is thinking...",
            })
    ...
    async def handle_live_ws(self, request: web.Request) -> web.WebSocketResponse:
        ...
        async def execute_and_reply_tool(session, call, captured_epoch: int):
            ...
            if is_stale:
                logger.info("Tool result for %s was interrupted/superseded (epoch %d -> %d)", name, captured_epoch, turn_epoch)
                response_payload = {"result": "Action cancelled or superseded by developer interruption."}
            else:
                        ...
                        while is_alive and not ws.closed:
                            ...
                            if response.tool_call:
                                for call in response.tool_call.function_calls:
                                    t = asyncio.create_task(execute_and_reply_tool(session, call, turn_epoch))
                            ...
                            if response.server_content:
                                ...
                                if it and it.text:
                                    await safe_client_send_json({"type": "user_transcript", "text": it.text})

                                ot = getattr(sc, "output_transcription", None)
                                if ot and ot.text:
                                    await safe_client_send_json({"type": "model_transcript", "text": ot.text})

                                # Forward thinking chunks and streaming audio
                                if sc.model_turn:
                                    for part in sc.model_turn.parts:
                                        if getattr(part, "thought", False) and part.text:
                                            await safe_client_send_json({"type": "model_thinking", "text": part.text})
                                        ...
                                        if silence_count >= 25 or len(speech_buffer) >= 500:
                                            ...
                                            if stt_engine and len(audio_bytes) >= 9600:
                                                try:
                                                    ...
                                                    text = await asyncio.to_thread(stt_engine.transcribe, samples)
                                                    text = text.strip() if text else ""
                                                    if text and is_alive:
                                                        logger.info("Live STT bridge transcribed: %s", text)
                                                        await safe_client_send_json({"type": "user_transcript", "text": text})
                                                        async with session_send_lock:
                                                            await session.send_realtime_input(text=text)
                                                except Exception as stt_err:
                                                    logger.debug("Live STT bridge error: %s", stt_err)
                    ...
                    try:
                        ...
                        if msg_type == "user_voice_command":
                            ...
                            if text:
                                ...
                                if intent_mode in ("speed", "comic") or engine in ("speed", "comic", "gemini_live"):
                                    try:
                                        from voicefi.integrations.gemini_live import GeminiLiveRunner
                                        is_comic = (intent_mode == "comic" or engine == "comic")
                                        voice = payload.get("voice") or ("Puck" if is_comic else "Aoede")
                                        mode = "comedy" if is_comic else "assistant"
                                        enable_sfx = is_comic
                                        self.broadcast_event({
                                            "type": "agent_thinking",
                                        ...
                                        play_audio = bool(payload.get("play_audio_on_server", payload.get("play_audio", False)))
                                        runner = GeminiLiveRunner(
                                            voice=voice,
                                            mode=mode,
                                            enable_sfx=enable_sfx,
                                        ...
                                        logger.error("Error running Gemini Live via WS: %s", e, exc_info=True)
                                    continue
                            ...
                            if payload.get("is_mobile") is True:
                                self.mobile_websockets.add(ws)
                            elif payload.get("is_mobile") is False and not is_remote_ip and not is_mobile_ua:
        ...
        for step in new_steps:
            ...
            if (
                ...
                try:
                    ...
                    pass
                max_words = getattr(getattr(fresh_cfg, "antigravity", None), "max_spoken_words", 300) or 300
